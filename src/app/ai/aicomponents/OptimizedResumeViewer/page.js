...
export default function OptimizedResumeViewer({ output: initialOutput }) {
  ...
  const [output, setOutput] = useState(JSON.parse(JSON.stringify(initialOutput)));
...
const handleGenerateAI = async () => {
  ...
  try {
    ...
    console.log('Full AI JSON:', json); // helpful debug

    // Try to pull just the first candidate text safely
    let text = json?.candidates?.[0]?.content?.parts?.[0]?.text || "";

    // If somehow not there, try other levels
    ...
    // Advanced cleaning: remove common markdown characters globally
    text = text
  ...
  // helper to safely update nested fields
...
// export default function GeminiAIHelper({ apiKey }) {
//   const [prompt, setPrompt] = useState('');
//   const [aiResult, setAiResult] = useState('');
//   const [loading, setLoading] = useState(false);
